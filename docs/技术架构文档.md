...
**文件位置**: `src/lib/stores/mobx/`

状态管理采用 MobX 实现，提供响应式的状态管理和可追踪的数据流。MobX 的可观察对象和动作机制确保状态变化的一致性和可预测性。
...
| ------------------ | -------------- |
| rootStore.ts       | 根状态管理器   |
| systemsStore.ts    | 系统状态管理   |
| simulationStore.ts | 模拟状态管理   |
| controlStore.ts    | 控制状态管理   |
| turbineStore.ts    | 汽轮机状态管理 |
| faultStore.ts      | 故障状态管理   |

### 2.4 日志系统

**文件位置**: `src/lib/utils/`

日志系统提供全面的日志记录、存储、管理和分析功能，支持多级别日志、模块化分类、性能监控和Web Workers日志传输。
...
5. **新增日志模块**：在 `src/lib/utils/` 目录下扩展日志功能
...
系统实现了全面的日志记录系统，支持多级别日志、模块化分类、性能监控和Web Workers日志传输。
...
export function measurePerformance<T>(name: string, fn: () => T): T {
  ...
  logger.trackPerformance(name, end - start, ModuleType.CONTROLLER);
  return result;
}
```
...
| 日志过多     | 日志级别设置过低     | 提高日志级别，减少日志输出       |
| 日志存储满   | 日志存储大小设置过小 | 增加maxStorageSize配置           |
