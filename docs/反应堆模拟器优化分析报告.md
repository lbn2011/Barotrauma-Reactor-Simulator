...
```typescript
function calculateTrendIndicator(history: number[], currentValue: number): TrendIndicator {
...
function processAlarmData(data: AlarmDataInput): AlarmDataOutput {
  ...
  const unacknowledgedAlarms = alarms.filter((alarm) => !acknowledgedAlarms.includes(alarm.id));

  const criticalAlarms = unacknowledgedAlarms.filter((alarm) => alarm.severity === 'critical');
...
function processTrendData(data: TrendDataInput): TrendDataOutput {
  ...
  const recentHistory = history.filter((entry) => entry.timestamp >= Date.now() - timeWindow);
...
- `g`：能群索引（g = 1, 2, ..., G）
- `D_g`：扩散系数（D*g = 1/(3Σ*{tr,g})，Σ\_{tr,g}是输运截面）
- `Σ_{a,g}`：吸收截面
...
### 9.7.4 反应堆再循环泵（maincp1.cs & maincp2.cs）
...
### 9.7.5 应急冷却泵系统（ECC_CONTROL.cs & SD_COOLING.cs）
...
### 9.7.6 汽轮机控制系统（turbine_s_ctrl.cs）
...
### 9.9.3 再循环泵面板（maincp1.cs & maincp2.cs）
...
### 9.9.5 汽轮机控制面板（turbine_s_ctrl.cs）
...
### 9.9.9 HEPA过滤器控制面板（HEPA_FLTR.cs）
...
### 9.9.10 除氧器控制面板（deaer_P_CTRL.cs & deaer_L_CTRL.cs）
...
#### 10.3.1 反应堆功率调节面板（RX_CONTROL.cs）
...
#### 10.3.2 反应堆再循环泵面板（maincp1.cs & maincp2.cs）
...
#### 10.3.3 应急冷却泵面板（ECC_CONTROL.cs & SD_COOLING.cs）
...
#### 10.3.4 汽轮机控制面板（turbine_s_ctrl.cs）
...
#### 10.3.7 除氧器面板（deaer_P_CTRL.cs & deaer_L_CTRL.cs）
