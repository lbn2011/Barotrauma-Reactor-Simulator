...
export function calculateEnergyBalance(input: EnergyBalanceInput): EnergyBalanceOutput {
  ...
  // 汽轮机系统能量平衡
  const P_mechanical = m_steam * (input.h_inlet - input.h_outlet) * input.η_turbine;
...
export function calculateMassBalance(input: MassBalanceInput): MassBalanceOutput {
  ...
  // 除氧器系统质量平衡
  const dM_deaerator = input.m_condensate + input.m_steam_heating - input.m_feedwater_out;
...
const totalReactivity =
  voidCoefficientResult.ρ_void + xenonPoisoningResult.ρ_Xe + controlRodResult.ρ_rod;
