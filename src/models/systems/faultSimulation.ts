...
export class FaultSimulation {
  ...
  static generateRandomFault (params: FaultSimulationParams): Fault | null {
    ...
    const weightedFaults = this.predefinedFaults.map((fault) => ({
      ...fault,
      weightedProbability:
        fault.probability * (1 - params.systemStatus.maintenanceLevel / 100),
    }));

    const totalWeight = weightedFaults.reduce(
      (sum, fault) => sum + fault.weightedProbability,
      0
    );
    ...
    for (const fault of weightedFaults) {
      ...
      if (randomWeight <= 0) {
        log.warn(
          `Random fault generated: ${fault.description} (${fault.severity})`
        );
        return fault;
      }
    }
  ...
  // 计算系统影响
  private static calculateSystemImpact (
    faults: Fault[]
  ): FaultSimulationResult['systemImpact'] {
  ...
  // 计算故障概率
  private static calculateFaultProbabilities (
    params: FaultSimulationParams
  ): FaultSimulationResult['faultProbabilities'] {
    const systemFaults = {
      cooling: 0.0,
      control: 0.0,
      steam: 0.0,
    ...
    for (const fault of this.predefinedFaults) {
      systemFaults[fault.affectedSystem as keyof typeof systemFaults] +=
        fault.probability;
    }
    ...
    return Object.entries(systemFaults).map(([system, probability]) => ({
      system,
      probability:
        probability * (1 - params.systemStatus.maintenanceLevel / 100),
    }));
  }
}
