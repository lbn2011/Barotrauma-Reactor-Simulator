...
export class ReactorCore {
	...
	static getCoreStatus(params: ReactorCoreParams): ReactorCoreStatus {
		...
		// 计算温度上升
		const temperatureRise = params.temperature.coolant - params.cooling.inletTemperature;
		
		// 计算压力差
		const pressureDifference = params.pressure.primary - params.pressure.secondary;
		...
		if (params.power.thermalPower > 1200 || params.temperature.fuel > 700 || params.pressure.primary > 8000000) {
			overallStatus = 'critical';
		} else if (params.power.thermalPower > 1000 || params.temperature.fuel > 600 || params.pressure.primary > 7000000) {
	...
	// 计算堆芯功率分布
	static calculatePowerDistribution(height: number, radius: number): number[][] {
		// 简化的功率分布计算
		// 假设功率分布为余弦分布
		const gridSize = 10;
	...
	// 计算堆芯冷却需求
	static calculateCoolingRequirement(thermalPower: number, inletTemperature: number, outletTemperatureLimit: number): number {
		// 计算冷却需求
		// Q = m * Cp * deltaT
		// m = Q / (Cp * deltaT)
