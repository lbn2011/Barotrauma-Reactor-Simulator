...
<div class="panel-container">
  ...
  <div class="pumps-container">
    ...
    <div class="pump-card">
      ...
      <div class="pump-status">
        ...
        ></div>
        <span class="status-label">{emergencyCoolingPumps.pump1.status ? '运行中' : '已停止'}</span>
        <button
    ...
    <div class="pump-card">
      ...
      <div class="pump-status">
        ...
        ></div>
        <span class="status-label">{emergencyCoolingPumps.pump2.status ? '运行中' : '已停止'}</span>
        <button
  ...
  <div class="pumps-container">
    <div class="pump-card">
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
        {#each [1, 2, 3] as valveNumber (valveNumber)}
          <div>
            <div class="status-label">注水阀 {valveNumber}</div>
            <button
              class={`toggle-btn ${Math.random() > 0.5 ? 'start' : 'stop'}`}
              on:click={() => console.log(`Safety injection valve ${valveNumber} toggled`)}
            >
  ...
  <div class="pump-diagram">
    ...
    <div class="diagram-content">
      <div class="pump-illustration">
        <div class={`pump-symbol ${emergencyCoolingPumps.pump1.status ? 'running' : ''}`}></div>
        <div class="pump-label">
          泵 1 - {emergencyCoolingPumps.pump1.status ? '运行中' : '已停止'}
        </div>
      </div>
      <div class="pump-illustration">
        <div class={`pump-symbol ${emergencyCoolingPumps.pump2.status ? 'running' : ''}`}></div>
        <div class="pump-label">
  ...
  <div class="status-indicators">
    ...
    <div class="indicator-card">
      ...
      <div class="indicator-value">
        {Number(emergencyCoolingPumps.pump1.status) + Number(emergencyCoolingPumps.pump2.status)}
      </div>
    </div>
    <div class="indicator-card">
      <div class="indicator-label">总冷却流量</div>
      <div class="indicator-value">
        {(emergencyCoolingPumps.pump1.flowRate + emergencyCoolingPumps.pump2.flowRate).toFixed(1)}%
      </div>
    </div>
    <div class="indicator-card">
  ...
  <div style="margin-top: 3rem;">
    ...
    >
      <ul style="margin: 0; padding-left: 1.5rem; color: #e0e0e0;">
        ...
        <li style="margin-bottom: 0.5rem;">启动后，反应堆功率会迅速下降，请做好相应准备</li>
        <li style="margin-bottom: 0.5rem;">正常运行时，应急冷却泵应保持停止状态</li>
        <li style="margin-bottom: 0.5rem;">安全注水阀用于在LOCA（失水事故）时快速注入冷却水</li>
        <li style="margin-bottom: 0.5rem;">
