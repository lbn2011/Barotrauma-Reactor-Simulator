  ...
  <div class="pumps-container">
    ...
    <div class="pump-card">
      ...
      <div class="pump-status">
        ...
        ></div>
        <span class="status-label">{recirculationPumps.pump1.status ? '运行中' : '已停止'}</span>
        <button
    ...
    <div class="pump-card">
      ...
      <div class="pump-status">
        ...
        ></div>
        <span class="status-label">{recirculationPumps.pump2.status ? '运行中' : '已停止'}</span>
        <button
  ...
  <div class="pump-diagram">
    ...
    <div class="diagram-content">
      <div class="pump-illustration">
        <div class={`pump-symbol ${recirculationPumps.pump1.status ? 'running' : ''}`}></div>
        <div class="pump-label">
          泵 1 - {recirculationPumps.pump1.status ? '运行中' : '已停止'}
        </div>
      </div>
      <div class="pump-illustration">
        <div class={`pump-symbol ${recirculationPumps.pump2.status ? 'running' : ''}`}></div>
        <div class="pump-label">
  ...
  <div class="status-indicators">
    <div class="indicator-card">
      ...
      <div class="indicator-value">
        {((recirculationPumps.pump1.speed + recirculationPumps.pump2.speed) / 2).toFixed(1)}%
      </div>
    </div>
    ...
    <div class="indicator-card">
      ...
      <div class="indicator-value">
        {Number(recirculationPumps.pump1.status) + Number(recirculationPumps.pump2.status)}
      </div>
    </div>
  </div>
  ...
  <div style="margin-top: 3rem;">
    ...
    >
      <ul style="margin: 0; padding-left: 1.5rem; color: #e0e0e0;">
        <li style="margin-bottom: 0.5rem;">再循环泵用于控制反应堆堆芯的冷却剂流量</li>
        <li style="margin-bottom: 0.5rem;">转速越高，冷却剂流量越大，反应堆功率潜力越大</li>
        <li style="margin-bottom: 0.5rem;">正常运行时，至少应保持一台泵运行</li>
        <li style="margin-bottom: 0.5rem;">泵的状态会影响反应堆的功率响应速度</li>
