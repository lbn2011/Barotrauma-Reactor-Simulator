...
reactorStore.subscribe((state) => {
  log.trace('Reactor state update received', {
    controlRods: state.controlRods.position,
...
function handleAutoModeToggle() {
  ...
  log.info(
    `Toggling control rod mode from ${currentMode ? 'auto' : 'manual'} to ${newMode ? 'auto' : 'manual'}`
  );
  toggleControlRodAutoMode();
...
<div class="panel-container">
  ...
  <div class="control-rod-section">
    ...
    <div class="position-control">
      ...
      <div class="quick-controls">
        <button class="quick-btn" on:click={() => setPositionQuickly(0)}> 完全抽出 </button>
        <button class="quick-btn" on:click={() => setPositionQuickly(25)}> 25% </button>
        <button class="quick-btn" on:click={() => setPositionQuickly(50)}> 50% </button>
        <button class="quick-btn" on:click={() => setPositionQuickly(75)}> 75% </button>
        <button class="quick-btn" on:click={() => setPositionQuickly(100)}> 完全插入 </button>
      </div>
  ...
  <div class="mode-indicator">
    ...
    <span class={`mode-badge ${controlRods.autoMode ? 'mode-auto' : 'mode-manual'}`}>
    ...
    <div class="rod-container">
      <div class="rod-guide">
        <div class="rod" style="height: ${100 - controlRods.position}%;">
          <div class="rod-inserted" style="height: ${controlRods.position}%;"></div>
        </div>
      </div>
    </div>
    <div class="scale">
  ...
  <div class="control-rod-grid">
    ...
    <div class="rod-grid">
      {#if controlRods.rods}
        {#each controlRods.rods as row, rowIndex (rowIndex)}
          {#each row as rod, colIndex (colIndex)}
            <div class="rod-cell">
              ...
              ></div>
              <div class="rod-type-tag" style="background-color: {getRodTypeColor(rod.type)}">
              ...
              <input
                ...
                class="rod-slider"
              />
              <button class="fuel-reload-btn" on:click={() => handleFuelReload(rowIndex, colIndex)}>
                燃料更换
              </button>
            </div>
          {/each}
  ...
  <div class="control-rod-section" style="margin-top: 3rem;">
    ...
    >
      <ul style="margin: 0; padding-left: 1.5rem; color: #e0e0e0;">
        <li style="margin-bottom: 0.5rem;">控制棒位置范围: 0% (完全抽出) 到 100% (完全插入)</li>
        <li style="margin-bottom: 0.5rem;">控制棒插入越深，反应性越低，反应堆功率下降</li>
        <li style="margin-bottom: 0.5rem;">控制棒抽出越多，反应性越高，反应堆功率上升</li>
        <li style="margin-bottom: 0.5rem;">正常运行时，控制棒位置通常保持在 40-60% 之间</li>
        <li style="margin-bottom: 0.5rem;">
          紧急插入 (AZ-5) 会将所有控制棒快速插入，立即降低反应堆功率
        </li>
        <li style="margin-bottom: 0.5rem;">自动模式下，系统会根据功率设定点自动调整控制棒位置</li>
