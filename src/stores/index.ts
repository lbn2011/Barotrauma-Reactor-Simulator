...
import { reactorStore, updateReactorState, startSimulation, stopSimulation, resetSimulation } from '../lib/stores/reactorStore';
  ...
  switch (intent.type) {
    ...
    case ReactorIntentType.SET_CONTROL_ROD_POSITION:
      if (intent.payload?.position !== undefined) {
        import('../lib/stores/reactorStore').then(({ setControlRodPosition }) => {
    ...
    case ReactorIntentType.TOGGLE_AUTOMATIC_CONTROL:
      import('../lib/stores/reactorStore').then(({ toggleAutomaticControl }) => {
    ...
    case UIIntentType.SET_REDUCED_MOTION:
      if (intent.payload?.enabled !== undefined) {
        ...
        localStorage.setItem('reducedMotion', intent.payload.enabled.toString());
      }
      break;
