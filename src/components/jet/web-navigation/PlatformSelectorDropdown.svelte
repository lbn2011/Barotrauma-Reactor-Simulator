...
<div class="platform-selector-dropdown">
  {#each platformSelectors as selector, index (index)}
    <FlowAction destination={selector.action}>
      <span>{selector.action.title}</span>
    </FlowAction>
  {/each}
</div>
