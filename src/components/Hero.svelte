...
<div class="hero">
  {#if video || artwork}
    ...
    >
      {#if video && !$prefersReducedMotion && (config.hero?.enableVideoAutoplay || false)}
        <Video loop autoplay useControls={false} {video} {profile} />
      {:else if artwork}
        <Artwork
          {artwork}
          {profile}
  ...
  {:else if collectionIcons && (config.hero?.enableCollectionIcons || false)}
    <ul class="app-icons">
      {#each collectionIcons?.slice(0, 5) as collectionIcon, index (index)}
        <li class="app-icon-container">
          <Artwork artwork={collectionIcon} profile="app-icon-large" />
        </li>
      {/each}
    </ul>
