...
function calculateMultiGroupDiffusion(params: any): any {
  ...
  // 简化的扩散方程（忽略空间依赖）
  const dφ_fast_dt = (νΣ_f_thermal * thermalFlux - Σ_a_fast * fastFlux - Σ_s_fast_to_thermal * fastFlux) / generationTime;
  const dφ_thermal_dt = (Σ_s_fast_to_thermal * fastFlux - Σ_a_thermal * thermalFlux) / generationTime;
...
function calculateVoidCoefficient(params: any): any {
  const { α_void, Δα, P_current, τ_delay } = params;
...
function calculateControlRodPhysics(params: any): any {
  const { position, ρ_max, z, L, ρ_graphite_effect } = params;
