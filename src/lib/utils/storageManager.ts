...
export class StorageManager {
	...
	async initialize(): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = (event) => {
				log.error('Failed to open database:', (event.target as IDBOpenDBRequest).error);
			...
			request.onupgradeneeded = (event) => {
				...
				if (!db.objectStoreNames.contains(this.STORES.SIMULATIONS)) {
					const simulationStore = db.createObjectStore(this.STORES.SIMULATIONS, {
						keyPath: 'id'
					});
					simulationStore.createIndex('name', 'name', { unique: false });
					simulationStore.createIndex('createdAt', 'createdAt', { unique: false });
					simulationStore.createIndex('updatedAt', 'updatedAt', { unique: false });
				...
				if (!db.objectStoreNames.contains(this.STORES.HISTORY)) {
					...
					historyStore.createIndex('simulationId', 'simulationId', { unique: false });
	...
	// 保存模拟状态
	async saveSimulation(simulation: Omit<SimulationState, 'id' | 'createdAt' | 'updatedAt'>): Promise<SimulationState> {
		if (!this.db) {
		...
		return new Promise((resolve, reject) => {
			const transaction = this.db!.transaction([this.STORES.SIMULATIONS], 'readwrite');
			const store = transaction.objectStore(this.STORES.SIMULATIONS);
			const request = store.add(newSimulation);
			
			request.onerror = (event) => {
				log.error('Failed to save simulation:', (event.target as IDBRequest).error);
	...
	async updateSimulation(id: string, updates: Partial<SimulationState>): Promise<SimulationState> {
		...
		return new Promise((resolve, reject) => {
			...
			getRequest.onerror = (event) => {
				log.error('Failed to get simulation:', (event.target as IDBRequest).error);
				reject((event.target as IDBRequest).error);
			};
			...
			getRequest.onsuccess = () => {
				...
				putRequest.onerror = (event) => {
					log.error('Failed to update simulation:', (event.target as IDBRequest).error);
					reject((event.target as IDBRequest).error);
				};
	...
	async getSimulation(id: string): Promise<SimulationState | null> {
		...
		return new Promise((resolve, reject) => {
			const transaction = this.db!.transaction([this.STORES.SIMULATIONS], 'readonly');
			const store = transaction.objectStore(this.STORES.SIMULATIONS);
	...
	async getAllSimulations(): Promise<SimulationState[]> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = (event) => {
				log.error('Failed to get all simulations:', (event.target as IDBRequest).error);
				reject((event.target as IDBRequest).error);
			};
	...
	async deleteSimulation(id: string): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			const transaction = this.db!.transaction([this.STORES.SIMULATIONS, this.STORES.HISTORY], 'readwrite');
			...
			simulationRequest.onerror = (event) => {
				log.error('Failed to delete simulation:', (event.target as IDBRequest).error);
				reject((event.target as IDBRequest).error);
			};
	...
	// 保存模拟历史
	async saveHistory(history: Omit<SimulationHistory, 'id'>): Promise<SimulationHistory> {
		if (!this.db) {
			await this.initialize();
		}
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = (event) => {
				log.error('Failed to save history:', (event.target as IDBRequest).error);
				reject((event.target as IDBRequest).error);
			};
	...
	async getHistory(simulationId: string): Promise<SimulationHistory[]> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = (event) => {
				log.error('Failed to get history:', (event.target as IDBRequest).error);
				reject((event.target as IDBRequest).error);
			};
			
			request.onsuccess = () => {
				const history = request.result;
				log.trace(`Retrieved ${history.length} history records for simulation: ${simulationId}`);
				resolve(history);
			};
		});
	}
	...
	// 导出模拟数据
	async exportSimulation(simulationId: string, format: 'json' | 'csv'): Promise<string> {
	...
	async clearAll(): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			simulationRequest.onerror = (event) => {
				log.error('Failed to clear simulations:', (event.target as IDBRequest).error);
				reject((event.target as IDBRequest).error);
			};
			...
			historyRequest.onerror = (event) => {
				log.error('Failed to clear history:', (event.target as IDBRequest).error);
				reject((event.target as IDBRequest).error);
			};
