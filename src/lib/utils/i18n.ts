...
export class I18nManager {
	...
	setLocale(locale: string): void {
		if (!this.supportedLocales.includes(locale)) {
			log.warn(`Unsupported locale: ${locale}, using default: ${this.defaultLocale}`);
	...
	translate(key: string, options?: TranslateOptions): string {
		...
		// 尝试在当前语言中查找
		let translation = this.resources[this.currentLocale]?.[namespace]?.[actualKey];
		
		// 如果找不到，尝试在默认语言中查找
		if (!translation) {
			translation = this.resources[this.defaultLocale]?.[namespace]?.[actualKey];
		}
		...
		if (options?.interpolate) {
			translation = Object.entries(options.interpolate).reduce((result, [placeholder, value]) => {
	...
	// 添加翻译资源
	addResources(locale: string, resources: TranslationResources[typeof locale]): void {
