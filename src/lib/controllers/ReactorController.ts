...
export class ReactorController implements IReactorController {
	...
	setControlRodPosition(position: number) {
		...
		log.info(`Control rod position set to ${clampedPosition}%, reactivity: ${reactivity.toFixed(4)}`);
	}
	...
	togglePump(system: string, pump: string) {
		...
		log.info(`Pump ${pump} in system ${system} ${newStatus === 'on' ? 'started' : 'stopped'}`);
		showSystemStatus(`${system}系统 ${pump} 已${newStatus === 'on' ? '启动' : '停止'}`, newStatus === 'on' ? 'success' : 'info');
	}
	...
	toggleTurbine() {
		...
		showSystemStatus(`汽轮机已${newStatus === 'on' ? '启动' : '停止'}`, newStatus === 'on' ? 'success' : 'info');
	}
	...
	setSteamValvePosition(position: number) {
		...
		log.info(`Steam valve position set to ${clampedPosition}%, flow rate: ${steamFlowRate.toFixed(1)} kg/s`);
	}
	...
	setFeedwaterValvePosition(position: number) {
		...
		log.info(`Feedwater valve position set to ${clampedPosition}%, flow rate: ${feedwaterFlowRate.toFixed(1)} kg/s`);
	}
	...
	checkSystemStatus() {
		...
		// 检查控制棒位置
		if (reactorState.controlRods.position < 10 && physicsState.thermal.power > 1000) {
