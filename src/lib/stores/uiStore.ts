...
export function setDragging(isDragging: boolean, element: string | null = null) {
	uiStore.update(state => ({
		...state,
		interaction: {
			...state.interaction,
...
export function showSystemStatus(message: string, level: 'info' | 'warning' | 'error' | 'success' = 'info') {
	uiStore.update(state => ({
		...state,
		systemStatus: {
...
export function toggleInstructions(show?: boolean) {
	uiStore.update(state => ({
		...
		modals: {
			...state.modals,
			showInstructions: show !== undefined ? show : !state.modals.showInstructions,
		},
	}));
}
...
export function toggleProjectInfo(show?: boolean) {
	uiStore.update(state => ({
		...
		modals: {
			...state.modals,
			showProjectInfo: show !== undefined ? show : !state.modals.showProjectInfo,
		},
	}));
}
