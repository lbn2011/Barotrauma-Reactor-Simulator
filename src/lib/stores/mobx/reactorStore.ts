...
export class ReactorSystemStore {
	...
	setPumpStatus(system: string, pump: string, status: 'off' | 'on') {
		if (system in this.state && pump in this.state[system as keyof ReactorSystemState]) {
			const pumpState = this.state[system as keyof ReactorSystemState][pump as keyof typeof this.state[keyof ReactorSystemState]] as PumpState;
			if (pumpState) {
	...
	setFeedwaterValvePosition(position: number) {
		this.state.feedwaterSystem.valvePosition = Math.max(0, Math.min(100, position));
	}
