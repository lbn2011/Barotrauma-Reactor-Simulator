...
const initialState: ReactorState = {
  ...
  faultSimulation: {
    ...
    riskLevel: 'low',
    recommendedActions: ['Continue normal operation', 'Maintain regular maintenance'],
...
export function setSingleControlRodPosition (
  ...
  const clampedPosition = Math.max(0, Math.min(100, position));
  log.debug(`Setting single control rod position: row ${row}, column ${col}, position ${clampedPosition}%`);
  reactorStore.update((state) => {
    const newState = JSON.parse(JSON.stringify(state));
...
export function toggleIsolationValve (
  ...
  reactorStore.update((state) => {
    ...
    newState.feedwaterSystem.isolationValves[valve].position = newStatus ? 100 : 0;
...
export function setIsolationValvePosition (
  ...
  reactorStore.update((state) => {
    ...
    newState.feedwaterSystem.isolationValves[valve].status = clampedPosition > 0;
    log.debug(`Isolation valve ${valve} position set to ${clampedPosition}%, status: ${clampedPosition > 0 ? 'open' : 'closed'}`);
    return newState;
  });
}
...
export function toggleFeedwaterHeater (heater: 'heater1' | 'heater2') {
  ...
  reactorStore.update((state) => {
    ...
    log.debug(`Feedwater heater ${heater} ${newStatus ? 'enabled' : 'disabled'}`);
    return newState;
  });
}
...
export function toggleRecirculationPump (pumpNumber: 1 | 2) {
  ...
  reactorStore.update((state) => {
    ...
    log.debug(`Recirculation pump ${pumpNumber} ${newStatus ? 'started' : 'stopped'}`);
    return {
      ...state,
      recirculationPumps: {
...
export function setRecirculationPumpSpeed (pumpNumber: 1 | 2, speed: number) {
  ...
  log.info(`Setting recirculation pump ${pumpNumber} speed to ${clampedSpeed}%`);
  reactorStore.update((state) => ({
    ...state,
    recirculationPumps: {
      ...state.recirculationPumps,
...
export function toggleEmergencyCoolingPump (pumpNumber: 1 | 2) {
  ...
  reactorStore.update((state) => {
    ...
    log.debug(`Emergency cooling pump ${pumpNumber} ${newStatus ? 'started' : 'stopped'}`);
    return {
      ...state,
      emergencyCoolingPumps: {
...
export function setEmergencyCoolingPumpFlowRate (
  ...
  log.info(`Setting emergency cooling pump ${pumpNumber} flow rate to ${clampedFlowRate}%`);
  reactorStore.update((state) => ({
    ...state,
    emergencyCoolingPumps: {
      ...state.emergencyCoolingPumps,
...
export function toggleCondenserCirculationPump (pumpNumber: 1 | 2) {
  ...
  reactorStore.update((state) => {
    const newStatus = !state.condenserCirculationPumps[`pump${pumpNumber}`].status;
    log.debug(`Condenser circulation pump ${pumpNumber} ${newStatus ? 'started' : 'stopped'}`);
    return {
      ...state,
      condenserCirculationPumps: {
...
export function setCondenserCirculationPumpFlowRate (
  ...
  const clampedFlowRate = Math.max(0, Math.min(100, flowRate));
  log.info(`Setting condenser circulation pump ${pumpNumber} flow rate to ${clampedFlowRate}%`);
  reactorStore.update((state) => ({
    ...state,
    condenserCirculationPumps: {
      ...state.condenserCirculationPumps,
...
export function toggleReactorFeedPump (pumpNumber: 1 | 2) {
  ...
  reactorStore.update((state) => {
    ...
    log.debug(`Reactor feed pump ${pumpNumber} ${newStatus ? 'started' : 'stopped'}`);
    return {
      ...state,
      reactorFeedPumps: {
...
export function setReactorFeedPumpFlowRate (
  ...
  const clampedFlowRate = Math.max(0, Math.min(100, flowRate));
  log.info(`Setting reactor feed pump ${pumpNumber} flow rate to ${clampedFlowRate}%`);
  reactorStore.update((state) => ({
    ...state,
    reactorFeedPumps: {
      ...state.reactorFeedPumps,
...
export function setSteamBypassPressureSetpoint (pressureSetpoint: number) {
  const clampedPressureSetpoint = Math.max(6.0, Math.min(8.0, pressureSetpoint));
  log.info(`Setting steam bypass pressure setpoint to ${clampedPressureSetpoint} MPa`);
  reactorStore.update((state) => ({
    ...state,
    steamBypass: {
      ...state.steamBypass,
